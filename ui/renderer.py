...
COLORS = {
    ...
    "danger": (255, 70, 70),       # Correção do crash!
    "success": (100, 255, 100),    # Adicionado para semântica visual
    "warning": (255, 200, 50),     # Adicionado para alertas de status
...
# ==========================================
# MANAGERS E CACHES (Responsabilidades Isoladas)
# ==========================================
class FontManager:
...
class AssetManager:
    ...
    def get_image(cls, nome_peca: str, team: str, tam: int) -> Optional[pygame.Surface]:
        ...
        caminho_base = sys._MEIPASS if getattr(sys, 'frozen', False) else os.path.dirname(os.path.dirname(__file__)) # type: ignore
...
class VFXManager:
    """Gerente de Efeitos Especiais (Resolve o problema de instanciar centenas de Surfaces por frame)"""
    _cached_surfaces: Dict[Tuple[Tuple[int,int,int], int], pygame.Surface] = {}

    @classmethod
    def draw_tint(cls, ecra: pygame.Surface, color: Tuple[int,int,int], alpha: int, rect: pygame.Rect):
        key = (color, rect.width)
...
class RendererState:
    """Isola os caches globais associados a um estado em particular."""
    def __init__(self):
...
def draw_text_wrapped(ecra: pygame.Surface, text: str, font: pygame.font.Font, color: Tuple[int, int, int], x: int, y: int, max_width: int) -> int:
    """1.3 - Suporta \\n e protege contra palavras gigantes que ultrapassam a largura."""
...
def desenhar_texto_animado(ecra: pygame.Surface, texto: str, tx: int, base_y: int, pulsar: float, cor: Tuple[int,int,int], font: pygame.font.Font) -> None:
    """Evita duplicação no rendering do VFX dos Spells e Stuns"""
        ...
        for c in range(COLUNAS):
            ...
            if p:
                ...
                if img:
                    ...
                    # 1.2 Correção do getattr enganador
...
def desenhar_destaques_com_hover(ecra: pygame.Surface, gs: Any, casa_selecionada: Optional[Tuple[int,int]], hover_pos: Optional[Tuple[int,int]], tam_casa: int, off_x: int, off_y: int) -> None:
    ...
    for mr, mc in movimentos:
        ...
        if hover_pos == (mr, mc):
            pygame.draw.rect(ecra, COLORS["success"], rect, 3)
    ...
    for foco, info in stuns.items():
        ...
        for (aoe_r, aoe_c) in info["aoe"]:
            if aoe_r == foco_r and aoe_c == foco_c: continue
...
def desenhar_loja_dinamica(ecra: pygame.Surface, off_x: int, off_y: int, width: int, height: int, catalogo: list, pontos: int, peca_selecionada: Optional[str]) -> Tuple[dict, pygame.Rect]:
    # 1.5 Correção do tipo de retorno fantasma `None`
    ...
    for i, item in enumerate(catalogo):
        ...
        if y + altura_btn > btn_r_y - 10:
            break

        if item["cost"] > pontos:
            cor_btn, cor_txt = (60, 40, 40), COLORS["text_muted"]
        elif item["name"] == peca_selecionada:
            cor_btn, cor_txt = (100, 200, 100), COLORS["text"]
        else:
            cor_btn, cor_txt = (60, 60, 80), (220, 220, 220)
...
def desenhar_selecao_dificuldade(ecra: pygame.Surface, w: int, h: int, elo_atual: int) -> Tuple[pygame.Rect, pygame.Rect, pygame.Rect]:
    ...
    # 1.4 - CLAMP de Rácio ELO corrigido!
...
def desenhar_enciclopedia(ecra: pygame.Surface, w: int, h: int, catalogo: list) -> pygame.Rect:
    # Nota: Placeholder de Enciclopédia isolado
