...
def simular_um_jogo(seed):
    ...
    resultado = {
        "turnos": 0,
        "winner": None,
        "mortes_por_peca": Counter(),
        "spawns_realizados": Counter(),
        "abates_por_peca": Counter(),
        "valor_destruido_por_peca": Counter(),
        "stuns_aplicados": 0,
        "mortes_por_stun": 0,
        "heatmap": [[0 for _ in range(8)] for _ in range(8)],
        "tabuleiro_encravado": None # A nossa Caixa Negra
    }
    
    turnos = 0
    # Limite aumentado para 300
    while not gs.game_over and turnos < LIMITE_TURNOS:
        ...
        if best_move:
            ...
            if best_move["type"] == "attack" and alvo:
                ...
                # Pylance fix: usar get com lista vazia
            ...
            if not gs.game_over:
                gs.game_over = True
                gs.winner = "Bloqueio Total"
            break
            
    # CAIXA NEGRA: Gravar estado se bater no teto de 300 turnos
...
def correr_diagnostico_profundo(num_jogos=100):
    ...
    try:
        with concurrent.futures.ProcessPoolExecutor() as process_executor:
            ...
            for futuro in concurrent.futures.as_completed(futuros):
                ...
                if res.get("tabuleiro_encravado"):
                    log_encravados += res["tabuleiro_encravado"]
                
                for r in range(8):
                    for c in range(8):
                        heatmap[r][c] += res["heatmap"][r][c]
        ...
        print("\n\n⚠️ Processo interrompido (Ctrl+C)...")
        if executor:
    ...
    if log_encravados:
        with open("jogos_encravados_log.txt", "w", encoding="utf-8") as f:
            f.write(log_encravados)
        print("⚠️ Foram detetados jogos bloqueados! Vê o 'jogos_encravados_log.txt'.")

    dados_exportacao = {
        "estatisticas_gerais": {
            "partidas_simuladas": num_jogos,
            "turnos_medios": round(turnos_totais / num_jogos, 1),
            "causas_fim": dict(causas_fim)
        },
        "metricas_combate": {
            "mortes_totais_por_peca": dict(mortes_por_peca),
            "abates_realizados_por_peca": dict(abates_por_peca),
            "pontos_destruidos_por_peca": dict(valor_destruido_por_peca),
            "stuns_aplicados": stuns_aplicados,
            "mortes_por_stun": mortes_por_stun,
            "taxa_letalidade_stun": round((mortes_por_stun / max(1, stuns_aplicados)) * 100, 2) if stuns_aplicados > 0 else 0.0
        },
        "metricas_invocacao": {
            "spawns_realizados": dict(spawns_realizados)
        },
        "heatmap": heatmap
    }

    with open("telemetria_profunda.json", "w", encoding="utf-8") as f:
    ...
    with open("relatorio_telemetria.txt", "w", encoding="utf-8") as f:
        ...
        f.write("=================================\n\n")
        f.write("1. CAUSAS DE FIM DE JOGO\n")
        for causa, qtd in causas_fim.items():
            f.write(f" - {causa}: {qtd} vezes ({(qtd/num_jogos)*100:.1f}%)\n")
        
        f.write("\n2. LETALIDADE OFENSIVA (Quem mais MATA e DESTRÓI VALOR)\n")
        f.write(f"{'Peça':<15} | {'Abates':<8} | {'Valor Destruído':<15}\n")
        f.write("-" * 45 + "\n")
        for peca in abates_por_peca:
            f.write(f"{peca:<15} | {abates_por_peca[peca]:<8} | {valor_destruido_por_peca[peca]} pts\n")
