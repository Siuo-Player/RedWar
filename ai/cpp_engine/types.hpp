...
void ensure_hero_behaviors_loaded();void parse_rwen(const std::string&);uint64_t compute_initial_hash();Piece create_piece(const std::string&,char);uint64_t get_piece_zobrist_key(int,int,const Piece&);uint64_t get_effect_zobrist_key(int,int,const TileEffect&);void compute_initial_eval();void update_piece(int,int,const Piece&);int get_piece_value(const Piece&,int,int);void update_timers(UndoInfo&);void restore_timers(const UndoInfo&);UndoInfo make_move(const Move&);void unmake_move(const Move&,const UndoInfo&);std::vector<Move> generate_valid_moves(char);int evaluate_board();std::string search_best_move(int);
#endif
