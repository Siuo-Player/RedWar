...
cpdef int obter_bonus_posicional(object piece, int r, int c):
    # O motor RedWar usa um tabuleiro fixo 8x8. A versão anterior fazia
    # escalamento em float + max/min a cada consulta, embora r/c já estejam
    # no intervalo [0, 7]. Mantemos exatamente a mesma orientação.
    cdef list tabela = MAPA_TABELAS.get(piece.name, PST_DEFAULT)
    cdef int idx_r = 7 - r if piece.team == 'pretas' else r
    return tabela[idx_r][c]
