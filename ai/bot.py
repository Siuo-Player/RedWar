...
class CppEngineBot:
    """Controlador Ares que depende de um contrato de bridge explícito."""

    def __init__(self, nodes=10000, executable_path=None, bridge=None):
        if not isinstance(nodes, int) or isinstance(nodes, bool) or nodes <= 0:
            raise ValueError("nodes must be a positive integer")
        if bridge is not None and not isinstance(bridge, EngineBridge):
            raise TypeError("bridge must implement EngineBridge")
    ...
    @property
    def process(self):
        """Compatibility view for callers that inspected the old subprocess."""
        return getattr(self.bridge, "process", None)
    ...
    def _read_response(self) -> str | None:
        return self.bridge.read_response()
...
def gerar_bot_por_elo(elo):
    ...
    if elo < 1400:
        return BOT_INICIANTE
    if elo < 1900:
        return BOT_INTERMEDIO
    return BOT_AVANCADO
