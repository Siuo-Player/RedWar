...
def _assert_python_terminal(state: GameState, expected_winner: str | None) -> None:
    state.state_history = {}
    state.check_game_over()
    assert state.game_over is (expected_winner is not None)
    assert state.winner == expected_winner


def test_terminal_contract_matrix_matches_python_terminal_classes():
    mutual = GameState()
    _assert_python_terminal(mutual, "Aniquilação Mútua (Pretas Vencem no Desempate)")

    white_only = GameState()
    white_only.board[6][0] = criar_peca_por_nome("Geomancer", "brancas")
    _assert_python_terminal(white_only, "Aniquilação (Brancas Vencem)")

    black_only = GameState()
    black_only.board[1][0] = criar_peca_por_nome("Cleric", "pretas")
    _assert_python_terminal(black_only, "Aniquilação (Pretas Vencem)")
    ...
    _assert_python_terminal(blocked, "Brancas Vencem (Oponente Bloqueado)")
    ...
    _assert_python_terminal(twc49, None)
    ...
    assert native[0][1] < 0  # Python's mutual-annihilation rule awards the tiebreak to Black.
    assert native[1][1] > 0
    assert native[2][1] < 0
    assert native[3][1] < 0  # White to move is the blocked/loss side.
