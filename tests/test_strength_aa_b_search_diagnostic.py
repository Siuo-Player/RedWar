...
def test_frozen_engine_search_diagnostic():
    """Expose the native diagnostic hidden behind the bridge's bestmove 0000 fallback."""
    if not ENGINE.exists():
        subprocess.run(["python", "tools/scripts/build_cpp_engine.py"], cwd=ROOT, check=True)
    ...
    assert bestmoves[-1] != "bestmove 0000", (
        "Frozen engine reproduced the A/A-B zero-move result. "
