...
def test_stun_on_stunned_is_kill():
    gs = GameState()
    alvo = Bone('pretas')
    alvo.stun_timer = 1 # O alvo já começa atordoado
    gs.board[0][0] = alvo
    
    # Aplicamos um novo Stun no alvo (a partir de uma casa qualquer)
    gs.make_action((2, 2), (0, 0), action_type="stun")
    
    # A casa tem de estar vazia porque ele morreu
