...
def fixture_position() -> GameState:
    state = GameState()
...
def read_until_bestmove(bridge: SubprocessEngineBridge) -> list[str]:
    responses = []
    for _ in range(32):
...
def test_production_protocol_lifecycle(production_engine: Path):
    ...
    try:
        ...
        response = bridge.read_response(timeout=10)
        assert response is not None
        assert response.startswith("info string nnue available=")
        ...
        response = bridge.read_response(timeout=10)
        assert response is not None
        assert response.startswith("info score classical ")
        ...
        assert bridge.last_request is not None
        assert bridge.last_request.state_identity == expected_identity
        responses = read_until_bestmove(bridge)
        assert responses[-1].startswith("bestmove ")
        diagnostics = [r for r in responses if r.startswith("info string search diagnostics ")]
...
def test_production_protocol_unknown_command_is_nonfatal(production_engine: Path):
    bridge = SubprocessEngineBridge(str(production_engine))
    try:
        bridge.ensure_running()
        assert bridge.read_response(timeout=10) == "readyok"
