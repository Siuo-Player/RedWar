...
`FrostMage` costs 5 points but can apply the `nevada` spell at range 3. Under the RedWar rule that a second stun while the target remains stunned becomes a death, a single FrostMage can create an extreme tactical sequence: cast Nevada on a cluster now and, on the next available FrostMage turn, convert the same cluster into kills.

This makes FrostMage a deliberate adversarial benchmark for Ares. Ares must not evaluate the unit from material cost alone; it must recognise the tactical value of the multi-target Nevada spell and the future conversion into deaths.

## Benchmark position

`tools/analytics/frostmage_benchmark.py` uses a fixed 8x8 RWEN position with exactly five enemy Bones on the FrostMage Nevada spell area: C5, D4, D5, D6 and E5. The FrostMage at A5 targets D5 with `SPELL nevada`, affecting all five occupied squares in the area. Because all five targets are stunned by the first spell, the next Nevada cast can kill all five under the two-stun rule.
...
2. Add selective search extension after the spell event.
