...
When no legal move is available, the engine emits `bestmove 0000`.
...
Stops active search, clears all transposition-table entries, and emits `info string clearhash ok`.

### Evaluation / NNUE diagnostics

`eval classical`

Emits `info score classical <VALUE>`.
...
Malformed or unsupported commands do not terminate the engine process by themselves. The dispatcher reports `info string command error: <MESSAGE>`.

Unknown commands are reported as `info string unknown command: <COMMAND>`.

Unknown `go` forms are reported as `info string unknown go command: <ARGS>`.

Search exceptions are converted into `info string search error: <MESSAGE>` followed by `bestmove 0000`.
...
The native canonical read-back seam also exposes the native board hash as a diagnostic identity value.
...
The command is diagnostic/read-only: it must not mutate engine state.
