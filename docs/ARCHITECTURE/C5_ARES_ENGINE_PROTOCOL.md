...
Native actions use algebraic coordinates: `MOVE`, `ATTACK`, `STUN`, `SPAWN <hero>`, and `SPELL <spell>`. Python parses the wire form; `GameAction` is the typed application-level value object. FrostMage `STUN` is normalized to `SPELL`/`nevada` by the Python bot.
