...
def auditar_relatorio_pricing(report: dict) -> dict:
    ...
    for change in changes:
        ...
        if "samples" not in change or "match_samples" not in change:
            ...
            raise ValueError(f"Hero {change.get('hero', 'unknown')} has occurrence samples below match samples")
    if missing_match_coverage:
        warnings.append("report does not expose independent match coverage per hero: " + ", ".join(map(str, missing_match_coverage)))
    if invalid:
