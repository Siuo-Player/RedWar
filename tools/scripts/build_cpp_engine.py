...
def compile_cpp_project(mode: str = "engine") -> Path:
    if mode == "engine":
        ...
        sources = PERFT_SOURCES
        test_path = TESTS_DIR / "cpp_perft_bridge_test.cpp"
        if not test_path.is_file():
            raise FileNotFoundError(f"Teste C++ em falta: {test_path}")
        sources = [*sources, str(test_path)]
        suffix = ".exe" if platform.system() == "Windows" else ""
...
def main() -> int:
    ...
    group.add_argument("--reversibility-test", action="store_true", help="Compila a regressão completa make/unmake do estado de pesquisa")
